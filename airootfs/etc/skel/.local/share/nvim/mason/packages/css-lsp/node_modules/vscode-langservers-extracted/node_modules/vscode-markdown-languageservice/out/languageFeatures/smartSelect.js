...
class MdSelectionRangeProvider {
    ...
    #linkProvider;
    #logger;
    constructor(parser, tocProvider, documentLink, logger) {
        this.#parser = parser;
...
function createHeaderRange(header, isClosestHeaderToPosition, onHeaderLine, parent, startOfChildRange) {
    ...
    const contentRange = lsp.Range.create((0, position_1.translatePosition)(range.start, { lineDelta: 1 }), range.end);
...
function createBlockRange(block, document, cursorLine, parent) {
    ...
    const range = lsp.Range.create(startLine, 0, endLine, (0, textDocument_1.getLine)(document, endLine).length);
...
async function createInlineRange(document, cursorPosition, parent, linkProvider, token) {
...
function createFencedRange(token, cursorLine, document, parent) {
    ...
    const fenceRange = lsp.Range.create(startLine, 0, endLine, (0, textDocument_1.getLine)(document, endLine).length);
    const contentRange = endLine - startLine > 2 && !onFenceLine ? lsp.Range.create(startLine + 1, 0, endLine - 1, (0, textDocument_1.getLine)(document, endLine - 1).length) : undefined;
    if (contentRange) {
...
async function createLinkRange(document, cursorPos, parent, linkProvider, token) {
    const links = await linkProvider.getLinks(document);
    if (token.isCancellationRequested) {
        return;
    }
    const link = links.links.find(link => (0, range_1.rangeContains)(link.source.range, cursorPos));
    if (!link) {
        return;
    }
    if (link.kind === documentLink_1.MdLinkKind.AutoLink) {
        return makeSelectionRange(link.source.hrefRange, makeSelectionRange(link.source.range, parent));
    }
    if (link.href.kind === documentLink_1.HrefKind.Reference && (0, range_1.areRangesEqual)(link.source.targetRange, link.source.range)) {
    ...
    if ((0, range_1.rangeContains)(link.source.targetRange, cursorPos)) {
        // Inside the href
        if (link.kind === documentLink_1.MdLinkKind.Definition) {
        ...
        const linkDestRanges = makeSelectionRange(lsp.Range.create((0, position_1.translatePosition)(link.source.targetRange.start, { characterDelta: 1 }), (0, position_1.translatePosition)(link.source.targetRange.end, { characterDelta: -1 })), makeSelectionRange(link.source.targetRange, fullLinkSelectionRange));
        if (link.source.titleRange) {
            // If we're inside the title, create a range for the title
            if ((0, range_1.rangeContains)(link.source.titleRange, cursorPos)) {
        ...
        if (link.source.isAngleBracketLink) {
            ...
            if ((0, range_1.rangeContains)(link.source.hrefRange, cursorPos)) {
                return makeSelectionRange(link.source.hrefRange, makeSelectionRange(lsp.Range.create((0, position_1.translatePosition)(link.source.hrefRange.start, { characterDelta: -1 }), (0, position_1.translatePosition)(link.source.hrefRange.end, { characterDelta: 1 })), linkDestRanges));
            }
        }
        if (link.source.titleRange) {
        ...
        // Inside the text
        if (link.kind === documentLink_1.MdLinkKind.Definition) {
            return makeSelectionRange(lsp.Range.create((0, position_1.translatePosition)(link.source.range.start, { characterDelta: 1 }), (0, position_1.translatePosition)(link.source.targetRange.start, { characterDelta: -3 })), fullLinkSelectionRange);
        }
        return makeSelectionRange(lsp.Range.create((0, position_1.translatePosition)(link.source.range.start, { characterDelta: 1 }), (0, position_1.translatePosition)(link.source.targetRange.start, { characterDelta: -1 })), fullLinkSelectionRange);
    }
}
...
function makeSelectionRange(range, parent) {
    if (parent && (0, range_1.areRangesEqual)(parent.range, range)) {
        return parent;
    }
    return { range, parent };
}
