...
})(IncludeWorkspaceHeaderCompletions || (exports.IncludeWorkspaceHeaderCompletions = IncludeWorkspaceHeaderCompletions = {}));
...
class MdPathCompletionProvider {
    ...
    #getPathCompletionContext(document, position) {
        ...
        if (htmlAttributePrefixMatch?.groups) {
            const validPathAttrs = html_1.htmlTagPathAttrs.get(htmlAttributePrefixMatch.groups.tag.toUpperCase());
    ...
    #createCompletionContext(kind, position, prefix, suffix, isAngleBracketPath) {
        return {
            kind,
            linkPrefix: prefix,
            linkTextStartPosition: (0, position_1.translatePosition)(position, { characterDelta: -prefix.length }),
    ...
    async *#provideReferenceSuggestions(document, position, context, token) {
        ...
        const replacementRange = lsp.Range.create(insertionRange.start, (0, position_1.translatePosition)(position, { characterDelta: context.linkSuffix.length }));
    ...
    async *#provideWorkspaceHeaderSuggestions(document, position, context, insertionRange, token) {
        ...
        for (const [toDoc, toc] of tocs) {
            const isHeaderInCurrentDocument = (0, path_2.isSameResource)(toDoc, (0, textDocument_1.getDocUri)(document));
    ...
    #getPathDocumentation(uri, stat) {
        ...
        if (!stat.isDirectory) {
            ...
            switch ((0, media_1.getMediaPreviewType)(uri)) {
    ...
    #normalizeFileNameCompletion(name) {
        if (this.#configuration.preferredMdPathExtensionStyle === 'removeExtension') {
            if ((0, path_2.looksLikeMarkdownFilePath)(this.#configuration, name)) {
